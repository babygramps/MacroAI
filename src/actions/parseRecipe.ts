...
import type { NormalizedFood, USDASearchResponse, ParsedRecipe, ParsedRecipeIngredient } from '@/lib/types';
...
2. Determine the total yield (e.g., "8 cups", "6 servings", "makes 12")
3. Estimate the total weight in grams of the finished recipe
4. Calculate servings and per-serving size in grams
5. Break down all ingredients with estimated weights in grams
...
export async function parseRecipe(recipeText: string): Promise<ParsedRecipe | null> {
  ...
  console.info('Parsing recipe with Gemini...');
  const parsedRecipe = await parseRecipeWithGemini(trimmedText);
  
  if (!parsedRecipe || parsedRecipe.ingredients.length === 0) {
    console.error('Failed to parse recipe or no ingredients found');
    return null;
  }

  console.info(`Parsed ${parsedRecipe.ingredients.length} ingredients from recipe: ${parsedRecipe.name}`);
  ...
  if (ingredientResults.length === 0) {
    console.error('No ingredients could be resolved');
    return null;
  }
  ...
  console.info(`Recipe parsed: ${parsedRecipe.name}, ${ingredientResults.length} ingredients, ${totalYieldG}g total, ${parsedRecipe.servings} servings`);
